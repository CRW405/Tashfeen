...
\begin{sol}
  there are $2^6$ combinations per beginning since the first 4 positions are already decided. For both it would be $2(2^6)$
\end{sol}
...
\begin{sol}
  $\binom{2n}{2}$ can be stated as "select a combination of 2 things from a 2n things". $2\binom{n}{2}+n^{2}$ can be stated as "select a combination of 2 things from n things twice then select 1 thing from n twice". Since that can be simplified to "select 2 from 2n" and to fulfill that, you would have already fulfilled "select 1 from n twice" so it can be simplified to $\binom{2n}{2}$ whish is equal $\binom{2n}{2}$
\end{sol}
...
\begin{sol}
  There are $\binom{6}{3} \cdot \binom{8}{4}$ possible, since we just get the possible choices for group one and multiply it by the combinations for group 2
\end{sol}
...
\begin{sol}
  \begin{enumerate}[label=(\alph*)]
    \item $\binom{12}{4}$
    \item $(\binom{5}{2}\binom{3}{1}\binom{4}{1}) + (\binom{5}{1}\binom{3}{2}\binom{4}{1}) + (\binom{5}{1}\binom{3}{1}\binom{4}{2})$ add all cases where one class has two and others have one together
    \item $\binom{5}{4}+\binom{5}{3}\binom{7}{1}$ cases where all are seniors + cases where one is not a senior
    \item $\binom{12}{4}-\binom{7}{4}$ total cases - cases where there are no seniors
  \end{enumerate}
\end{sol}
