...
\begin{sol}
  \begin{enumerate}
  \item 0 - I have not taken this class or any other similiar classes ever
  \item 2 - Im just bad at Calculus 
  \item 1 - I am currently taking this one, but havent learned enough for this class
  \item 3
  \end{enumerate}
  Probability and Statistics and Linear Algebra are not listed as prerequisites for this class, this assignment seems to expect us to have taken these already.
\end{sol}

\question Of the sub-plots (a), (b), (c) and (d) in figure %\ref{cor}
, state
...
\begin{enumerate}
  \item $-y^{-2}sin(x^2)$
  \item $\frac{2xcos(x^2)}{y}$
  \item $[\frac{2xcos(x^2)}{y}, -y^{-2}sin(x^2)]$
\end{enumerate}
\end{sol}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \item Prove that the matrix product is not a symmetric relation \ie
        ...
        counter example?
        \begin{sol}
          In 9.c, AB is a valid operation, but BA is not since B does not have as many rows as A has columns
        \end{sol}
  ...
  \item For $\mathbf{X} \in \R^{n\times p}, \beta \in \R^p, y \in \R^n$
        ...
        \begin{sol}
          If we assume n = 1 and p = 1, then $\beta^T = \beta, X^T = X, y^T = y$ then $\beta^T\mathbf{X}^Ty = y^T\mathbf{X}\beta$ could be written as $\beta X y = y X \beta$ which is true
        \end{sol}
  ...
  \item Let $f(\beta) = \beta^T\beta$. Prove that the derivative $\frac{\D
            ...
            \begin{sol}
              if we assume $\beta = [x, x]$ then $\beta^T \beta = x^2 + x^2$ $\frac{df}{d\beta}= 2x + 2x$ which is equivelent to $2\beta$
            \end{sol}
\end{enumerate}
...
\begin{sol}
  0 or approaching 0 since $\sqrt{x}$ is irrational
\end{sol}

\question The code snippet in listing %\ref{read}
reads the plain text of the
